...
run_benchmark() {
  ...
  hyperfine \
    ...
    "taskset -c 1 perf script flamegraph -F 99 taskset -c 2-15 build/src/bitcoind -datadir=${TMP_DATADIR} \-connect=${connect_address} -daemon=0 -chain=${chain} -stopatheight=${stop_at_height} -dbcache=${dbcache} -printtoconsole=0" \
