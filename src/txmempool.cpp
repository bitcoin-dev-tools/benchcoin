...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, int64_t spendheight) const
{
    ...
    for (const auto& it : GetSortedDepthAndScore()) {
        ...
        std::vector<std::reference_wrapper<const Coin>> coins{mempoolDuplicate.AccessCoins(tx)};
        assert(Consensus::CheckTxInputs(tx, dummy_state, mempoolDuplicate, std::span<std::reference_wrapper<const Coin>>{coins}, spendheight, txfee));
