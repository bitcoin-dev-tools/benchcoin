...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache, CCoinsViewCache& active_coins_tip, ValidationCache& validation_cache) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    std::vector<CTxOut> spent_outputs{active_coins_tip.GetCoins(tx)};
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        std::vector<CTxOut> spent_outputs{m_node.chainman->ActiveChainstate().CoinsTip().GetCoins(CTransaction(spend_tx))};
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        TxValidationState state;
        PrecomputedTransactionData txdata;
        std::vector<CTxOut> spent_outputs{m_node.chainman->ActiveChainstate().CoinsTip().GetCoins(CTransaction(invalid_with_cltv_tx))};
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        TxValidationState state;
        PrecomputedTransactionData txdata;
        std::vector<CTxOut> spent_outputs{m_node.chainman->ActiveChainstate().CoinsTip().GetCoins(CTransaction(invalid_with_csv_tx))};
...

    {
        ...
        TxValidationState state;
        PrecomputedTransactionData txdata;
        std::vector<CTxOut> spent_outputs{m_node.chainman->ActiveChainstate().CoinsTip().GetCoins(CTransaction(tx))};
        txdata.Init(tx, std::move(spent_outputs));
