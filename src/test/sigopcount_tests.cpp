...
BOOST_AUTO_TEST_CASE(GetTxSigOpCost)
{
    ...
    // Multisig script (legacy counting)
    {
        ...
        std::vector<std::reference_wrapper<const Coin>> spending_coins{coins.AccessCoins(CTransaction(spendingTx))};
        // Legacy counting only includes signature operations in scriptSigs and scriptPubKeys
        // of a transaction and does not take the actual executed sig operations into account.
        // spendingTx in itself does not contain a signature operation.
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), std::span<std::reference_wrapper<const Coin>>{spending_coins}, flags) == 0);
        // creationTx contains two signature operations in its scriptPubKey, but legacy counting
        // is not accurate.
        std::vector<std::reference_wrapper<const Coin>> creation_coins{coins.AccessCoins(CTransaction(creationTx))};
        assert(GetTransactionSigOpCost(CTransaction(creationTx), std::span<std::reference_wrapper<const Coin>>{creation_coins}, flags) == MAX_PUBKEYS_PER_MULTISIG * WITNESS_SCALE_FACTOR);
    ...
    // Multisig nested in P2SH
    {
        ...
        BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, CScriptWitness());
        std::vector<std::reference_wrapper<const Coin>> spending_coins{coins.AccessCoins(CTransaction(spendingTx))};
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), std::span<std::reference_wrapper<const Coin>>{spending_coins}, flags) == 2 * WITNESS_SCALE_FACTOR);
        assert(VerifyWithFlag(CTransaction(creationTx), spendingTx, flags) == SCRIPT_ERR_CHECKMULTISIGVERIFY);
    }
    ...
    // P2WPKH witness program
    {
        ...
        std::vector<std::reference_wrapper<const Coin>> spending_coins{coins.AccessCoins(CTransaction(spendingTx))};
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), std::span<std::reference_wrapper<const Coin>>{spending_coins}, flags) == 1);
        // No signature operations if we don't verify the witness.
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), std::span<std::reference_wrapper<const Coin>>{spending_coins}, flags & ~SCRIPT_VERIFY_WITNESS) == 0);
    ...
    // P2WSH witness program
    {
        ...
        BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, scriptWitness);
        std::vector<std::reference_wrapper<const Coin>> spending_coins{coins.AccessCoins(CTransaction(spendingTx))};
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), std::span<std::reference_wrapper<const Coin>>{spending_coins}, flags) == 2);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), std::span<std::reference_wrapper<const Coin>>{spending_coins}, flags & ~SCRIPT_VERIFY_WITNESS) == 0);
        assert(VerifyWithFlag(CTransaction(creationTx), spendingTx, flags) == SCRIPT_ERR_CHECKMULTISIGVERIFY);
    }
