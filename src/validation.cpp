...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    if (VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *m_signature_cache, *txdata, nFlags), &error)) {
        return std::nullopt;
    } else {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Disable schnorr for experimental purposes
    if (chainman.m_options.disable_schnorr == true) {
        flags |= SCRIPT_DISABLE_SCHNORR_VERIFY;
    }
