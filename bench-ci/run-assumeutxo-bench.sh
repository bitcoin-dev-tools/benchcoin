...
prepare_assumeutxo_snapshot_run() {
  ...
  clean_datadir "${TMP_DATADIR}"
  build/src/bitcoind -datadir="${TMP_DATADIR}" -connect="${CONNECT_ADDRESS}" -daemon=0 -chain="${chain}" -stopatheight=1 -blocksxor=0
  build/src/bitcoind -datadir="${TMP_DATADIR}" -connect="${CONNECT_ADDRESS}" -daemon=0 -chain="${chain}" -dbcache=16000 -pausebackgroundsync=1 -loadutxosnapshot="${UTXO_PATH}" -blocksxor=0 || true
...
run_benchmark() {
  ...
  hyperfine \
    ...
    "perf script flamegraph build/src/bitcoind -datadir=${TMP_DATADIR} -connect=${connect_address} -daemon=0 -chain=${chain} -stopatheight=${stop_at_height} -blocksxor=0" \
